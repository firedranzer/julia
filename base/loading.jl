...
_include_from_serialized(from::Module, content::Vector{UInt8}, depmods::Vector{Module}) =
    ccall(:jl_restore_incremental_from_buf, Any,
          (Any, Ptr{UInt8}, Int, Any),
          from, content, sizeof(content), depmods)

_include_from_serialized(from::Module, path::String, depmods::Vector{Module}) =
...
function _require_from_serialized(from::Module, mod::Symbol, path_to_try::String)
    return _require_from_serialized(from, mod, path_to_try, parse_cache_header(path_to_try)[3])
end
function _require_from_serialized(from::Module, mod::Symbol, path_to_try::String, depmodnames::Vector{Pair{Symbol, UInt64}})
    ...
    restored = _include_from_serialized(from, path_to_try, depmods)
...
function _require_search_from_serialized(from::Module, mod::Symbol, sourcepath::String)
    ...
    for path_to_try in paths::Vector{String}
        ...
        restored = _require_from_serialized(from, mod, path_to_try, deps)
        if isa(restored, Exception)
